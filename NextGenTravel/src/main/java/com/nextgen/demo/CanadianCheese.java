package com.nextgen.demo;

public class CanadianCheese {
	private String cheeseId;
	private String cheeseNameEn;
	private String cheeseNameFr;
	private String ManufacturerNameEn;
	private String ManufacturerNameFr;
	private String ManufacturerProvCode;
	private String ManufacturingTypeEn;
	private String ManufacturingTypeFr;
	private String WebSiteEn;
	private String WebSiteFr;
	private String FatContentPercent;
	private String MoisturePercent;
	private String ParticularitiesEn;
	private String ParticularitiesFr;
	private String FlavourEn;
	private String FlavourFr;
	private String CharacteristicsEn;
	private String CharacteristicsFr;
	private String RipeningEn;
	private String RipeningFr;
	private String Organic;
	private String CategoryTypeEn;
	private String CategoryTypeFr;
	private String MilkTypeEn;
	private String MilkTypeFr;
	private String MilkTreatmentTypeEn;
	private String MilkTreatmentTypeFr;
	private String RindTypeEn;
	private String RindTypeFr;
	private String LastUpdateDate;
	
	public String getCheeseId() {
		return cheeseId;
	}
	public void setCheeseId(String cheeseId) {
		this.cheeseId = cheeseId;
	}
	public String getCheeseNameEn() {
		return cheeseNameEn;
	}
	public void setCheeseNameEn(String cheeseNameEn) {
		this.cheeseNameEn = cheeseNameEn;
	}
	public String getCheeseNameFr() {
		return cheeseNameFr;
	}
	public void setCheeseNameFr(String cheeseNameFr) {
		this.cheeseNameFr = cheeseNameFr;
	}
	public String getManufacturerNameEn() {
		return ManufacturerNameEn;
	}
	public void setManufacturerNameEn(String manufacturerNameEn) {
		ManufacturerNameEn = manufacturerNameEn;
	}
	public String getManufacturerNameFr() {
		return ManufacturerNameFr;
	}
	public void setManufacturerNameFr(String manufacturerNameFr) {
		ManufacturerNameFr = manufacturerNameFr;
	}
	public String getManufacturerProvCode() {
		return ManufacturerProvCode;
	}
	public void setManufacturerProvCode(String manufacturerProvCode) {
		ManufacturerProvCode = manufacturerProvCode;
	}
	public String getManufacturingTypeEn() {
		return ManufacturingTypeEn;
	}
	public void setManufacturingTypeEn(String manufacturingTypeEn) {
		ManufacturingTypeEn = manufacturingTypeEn;
	}
	public String getManufacturingTypeFr() {
		return ManufacturingTypeFr;
	}
	public void setManufacturingTypeFr(String manufacturingTypeFr) {
		ManufacturingTypeFr = manufacturingTypeFr;
	}
	public String getWebSiteEn() {
		return WebSiteEn;
	}
	public void setWebSiteEn(String webSiteEn) {
		WebSiteEn = webSiteEn;
	}
	public String getWebSiteFr() {
		return WebSiteFr;
	}
	public void setWebSiteFr(String webSiteFr) {
		WebSiteFr = webSiteFr;
	}
	public String getFatContentPercent() {
		return FatContentPercent;
	}
	public void setFatContentPercent(String fatContentPercent) {
		FatContentPercent = fatContentPercent;
	}
	public String getMoisturePercent() {
		return MoisturePercent;
	}
	public void setMoisturePercent(String moisturePercent) {
		MoisturePercent = moisturePercent;
	}
	public String getParticularitiesEn() {
		return ParticularitiesEn;
	}
	public void setParticularitiesEn(String particularitiesEn) {
		ParticularitiesEn = particularitiesEn;
	}
	public String getParticularitiesFr() {
		return ParticularitiesFr;
	}
	public void setParticularitiesFr(String particularitiesFr) {
		ParticularitiesFr = particularitiesFr;
	}
	public String getFlavourEn() {
		return FlavourEn;
	}
	public void setFlavourEn(String flavourEn) {
		FlavourEn = flavourEn;
	}
	public String getFlavourFr() {
		return FlavourFr;
	}
	public void setFlavourFr(String flavourFr) {
		FlavourFr = flavourFr;
	}
	public String getCharacteristicsEn() {
		return CharacteristicsEn;
	}
	public void setCharacteristicsEn(String characteristicsEn) {
		CharacteristicsEn = characteristicsEn;
	}
	public String getCharacteristicsFr() {
		return CharacteristicsFr;
	}
	public void setCharacteristicsFr(String characteristicsFr) {
		CharacteristicsFr = characteristicsFr;
	}
	public String getRipeningEn() {
		return RipeningEn;
	}
	public void setRipeningEn(String ripeningEn) {
		RipeningEn = ripeningEn;
	}
	public String getRipeningFr() {
		return RipeningFr;
	}
	public void setRipeningFr(String ripeningFr) {
		RipeningFr = ripeningFr;
	}
	public String getOrganic() {
		return Organic;
	}
	public void setOrganic(String organic) {
		Organic = organic;
	}
	public String getCategoryTypeEn() {
		return CategoryTypeEn;
	}
	public void setCategoryTypeEn(String categoryTypeEn) {
		CategoryTypeEn = categoryTypeEn;
	}
	public String getCategoryTypeFr() {
		return CategoryTypeFr;
	}
	public void setCategoryTypeFr(String categoryTypeFr) {
		CategoryTypeFr = categoryTypeFr;
	}
	public String getMilkTypeEn() {
		return MilkTypeEn;
	}
	public void setMilkTypeEn(String milkTypeEn) {
		MilkTypeEn = milkTypeEn;
	}
	public String getMilkTypeFr() {
		return MilkTypeFr;
	}
	public void setMilkTypeFr(String milkTypeFr) {
		MilkTypeFr = milkTypeFr;
	}
	public String getMilkTreatmentTypeEn() {
		return MilkTreatmentTypeEn;
	}
	public void setMilkTreatmentTypeEn(String milkTreatmentTypeEn) {
		MilkTreatmentTypeEn = milkTreatmentTypeEn;
	}
	public String getMilkTreatmentTypeFr() {
		return MilkTreatmentTypeFr;
	}
	public void setMilkTreatmentTypeFr(String milkTreatmentTypeFr) {
		MilkTreatmentTypeFr = milkTreatmentTypeFr;
	}
	public String getRindTypeEn() {
		return RindTypeEn;
	}
	public void setRindTypeEn(String rindTypeEn) {
		RindTypeEn = rindTypeEn;
	}
	public String getRindTypeFr() {
		return RindTypeFr;
	}
	public void setRindTypeFr(String rindTypeFr) {
		RindTypeFr = rindTypeFr;
	}
	public String getLastUpdateDate() {
		return LastUpdateDate;
	}
	public void setLastUpdateDate(String lastUpdateDate) {
		LastUpdateDate = lastUpdateDate;
	}
	
	
}
